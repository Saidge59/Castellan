package system
{
	...
	public class PathFinder
	{
		...
		{		
			var col:Number = (ax+1)%2;
			var coll:Number = ax%2;
			
			// Если клеточка слева-ввурху свободна
			if (inMap(ax - 1, ay - coll) && _mapMask[ay - coll][ax - 1] == _freeCell)
			{
				_mapMask[ay - coll][ax - 1] = WATER_KEY; // Заполняем её водой
				// Запоминаем из какой клетки вода пришла
				(_mapDirs[ay - coll][ax - 1] as Point).x = ax;
				(_mapDirs[ay - coll][ax - 1] as Point).y = ay;
			}
			
			// Если клеточка слева-снизу свободна
			if (inMap(ax - 1, ay + col) && _mapMask[ay + col][ax - 1] == _freeCell)
			{
				_mapMask[ay + col][ax - 1] = WATER_KEY; // Заполняем её водой
				// Запоминаем из какой клетки вода пришла
				(_mapDirs[ay + col][ax - 1] as Point).x = ax;
				(_mapDirs[ay + col][ax - 1] as Point).y = ay;
			}
				
			// Есле клеточка справа-ввурху свободна
			if (inMap(ax + 1, ay - coll) && _mapMask[ay - coll][ax + 1] == _freeCell)
			{
				_mapMask[ay - coll][ax + 1] = WATER_KEY; // Заполняем её водой
				// Запоминаем из какой клетки вода пришла
			...
			// Если клеточка справа-снизу свободна
			if (inMap(ax + 1, ay + col) && _mapMask[ay + col][ax + 1] == _freeCell)
			{
				_mapMask[ay + col][ax + 1] = WATER_KEY; // Заполняем её водой
				// Запоминаем из какой клетки вода пришла
